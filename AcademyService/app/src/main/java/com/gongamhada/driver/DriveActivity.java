...
public class DriveActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        endDriveURL = getString(R.string.url) + "driver/end?";
        ...
        busstopIcon = BitmapFactory.decodeResource(this.getResources(), R.drawable.busstop);
        busicon = Bitmap.createScaledBitmap(busicon, 200, 200, false);
        studentIcon = Bitmap.createScaledBitmap(studentIcon, 75, 75, false);
        busstopIcon = Bitmap.createScaledBitmap(busstopIcon, 75, 75, false);
                    ...
                    public void run() {
                        ...
                        if (mMap != null) {
                            if (myLocationMarker == null) {
                                ...
                                locationMarkerOption.icon(BitmapDescriptorFactory.fromBitmap(busicon));
    ...
    public void requestCheckPoint(int index) {
        ...
        for (int i = 0; i < studentList.size(); i++) {
            if (studentList.get(i).check == true) {
                ...
                contentValues.put("stdntNo", studentList.get(i).acc_id);
        ...
        try {
            ...
            if (resultJson.getString("status").equals("REJECT") == true) {
                ...
                if(studentMarker == null){
                    ...
                    markerOptions.icon(BitmapDescriptorFactory.fromBitmap(studentIcon));
                    studentMarker = mMap.addMarker(markerOptions);
                }
                else {
            ...
            e.printStackTrace();
            Intent intent = new Intent(DriveActivity.this, PopupActivity.class);
            intent.putExtra("guide", "네트워크 오류. 인터넷을 확인 해주세요.");
            startActivity(intent);
        }
...
//        }
    ...
    public void initBusstopMarker(){
        for(int i = 0; i < course.pointList.size(); i++){
            double lat = course.pointList.get(i).latitude;
            double lng = course.pointList.get(i).longitude;
            Log.d("point_test", "" + lat + " , " + lng);
            MarkerOptions markerOptions = new MarkerOptions();
            markerOptions.position(new LatLng(lat, lng));
            markerOptions.icon(BitmapDescriptorFactory.fromBitmap(busstopIcon));
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        LatLng demoMark = new LatLng(course.pointList.get(2).latitude, course.pointList.get(2).longitude);
        ...
        initBusstopMarker();
//        MarkerOptions markerOptions = new MarkerOptions();
//        markerOptions.position(demoMark);
//
//        mMap.addMarker(markerOptions);
//        mMap.moveCamera(CameraUpdateFactory.newLatLng(demoMark));
//        mMap.animateCamera(CameraUpdateFactory.zoomTo(14));

        Polyline polyline = mMap.addPolyline((new PolylineOptions()).clickable(true).addAll(pathPointList));
        polyline.setColor(0xffDDDDDD);
